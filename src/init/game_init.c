...
/*   Updated: 2023/10/05 10:56:36 by purple           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	init_the_game(t_data *data)
{
	...
	data->base_img[0].image = mlx_new_image(data->mlx.mlx_id, \
	data->mlx.size.x, data->mlx.size.y);
	data->base_img[0].adress = mlx_get_data_addr(data->base_img[0].image, \
	&data->base_img[0].bpp, &data->base_img[0].line_lenght, \
	&data->base_img[0].endian);
	return (0);
}
...
void	init_orientation(t_data *data)
{
	data->ray.pos.x = data->bpose.x;
	data->ray.pos.y = data->bpose.y;
...
static int	path_texture(t_data *map)
{
	...
	width = (int)map->mlx.size.x;
	height = (int)map->mlx.size.y;
